<div class="yl1">
    <h2> Ülesanne 1.</h2>
    Lisa massivi 15 HTML värvi.
    Kuva tsükliga span ja color abil

    <?php
    //Masiiv HTML värvidest
    $colorHTML = Array(
        "Red",
        "Green",
        "Blue",
...
<div class="yl2">
    <h2>Teosta järgmised protseduurid: </h2>
    * Kuva kõik autod loeelu
...
echo '</ol>';
    ...
    echo "Autode arv: ".count($autod)."tk"."<br>";



    echo "<h2>kontrollida, kas massiviid on ühepikkused</h2>"."<br>";
    if (count($autod) == count($vimKood)){
        echo "Masiivid on ühepikulised";
    }
    else{
        echo "Masiivid ei ole Ühepikulised";
    }

    echo "<h2>leida eraldi Toyotade ja Audide arv nimekirjas</h2>";
    ...
    echo "Toyatade arv: ".$tArv."tk"."<br>";
    echo "Audide arv: ".$aArv."tk"."<br>";

    echo "<h2>leia ja väljasta VIN koodid kui alla 17</h2><br>";

    for ($l = 0; $l < count($vimKood); $l++){
        if (strlen($vimKood[$l]) < 17){
            echo $autod[$l]." - ".$vimKood[$l]."<br>";
        }
    }
    ?>
</div>
...
<div class="yl3">
    <h2>Ülessanne 3.</h2>
    Kuva img kaustats kõik pildid width=100px
    <br>
    ...
    $pildiArray = array();
    foreach ($pildiArray as $rida){
        array_push($pildiArray, $rida);
    }
    for ($i = 0; $i < count($pildiArray); $i++){
        echo "<img src='content/img/$pildiArray[$i].png' alt='$pildiArray[$i]'>";
    }
